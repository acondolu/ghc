...
says it has arity "infinity" the first time round. And that arity must
propagate to the arity of g, because that's what's ultimately tail-called.

This example happens a lot; it first showed up in Andy Gill's thesis,
fifteen years ago!  It also shows up in the code for 'rnf' on lists
in #4138. #18793 has a tail-called g like in the example above.
...
until it finds a stable arity type.
...
-- | One mode for each entry-point to the analyser, see the different
-- constructors.
--
-- The mode governs
--   1. How pedantic we are wrt. ⊥
